-- Fix blackjack_deal: replace bj_build_shoe/bj_draw calls with bj_card
-- The bj_build_shoe and bj_draw functions were never created in any migration,
-- causing "function bj_build_shoe(integer, text, text, integer) does not exist".
-- blackjack_action already uses bj_card directly, so we align blackjack_deal to do the same.
-- Also store next_idx in the result JSON (blackjack_action reads it from there).
