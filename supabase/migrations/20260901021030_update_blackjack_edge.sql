-- Update Blackjack (5% edge via blackjack_pays 1.2x instead of 1.5x)
...
v_deck := bj_build_shoe(6, v_server_seed, p_client_seed, v_nonce);

v_pc1 := bj_draw(v_deck, 0);
v_dc1 := bj_draw(v_deck, 1);
v_pc2 := bj_draw(v_deck, 2);
v_dc2 := bj_draw(v_deck, 3);
...
VALUES (v_session_id, v_server_seed, jsonb_build_object('deck', v_deck, 'next_idx', 4));
